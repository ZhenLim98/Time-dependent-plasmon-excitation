...
out = zeros(2 * Ng + 1);
    for i = 1:Ng
        ...
        upper_diag = 1i*sigma2( (om+(recVecs((i + 1):end)-i)*Omega),om, Omega,(recVecs((i + 1):end)), Ng)*alphaF./ ...
                     (e0*(om+(recVecs((i + 1):end)-i)*Omega));
        lower_diag = 1i*sigma2(om+(recVecs(1:(end-i))+i)*Omega, om,Omega, (recVecs(1:(end-i))), Ng)*alphaF./ ...
